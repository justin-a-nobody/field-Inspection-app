...
db.transaction((tx) => {
  tx.executeSql(
    'CREATE TABLE IF NOT EXISTS heading (variety_id TEXT PRIMARY KEY NOT NULL, variety TEXT,crop_id TEXT, FOREIGN KEY(crop_id)  REFERENCES crop(crop_id))',
    [],
    () => {
      console.log('heading table created successfully');
    },
    (error) => {
      console.error('Failed to create table:', error);
    }
...
db.transaction((tx) => {
  ...
  tx.executeSql(
    'CREATE TABLE IF NOT EXISTS answers (variet_type_id INT PRIMARY KEY NOT NULL, variety_type TEXT, description TEXT)',
    [],
    () => {
      console.log('answers type table created ')
...
db.transaction((tx) => {
  tx.executeSql(
    'CREATE TABLE IF NOT EXISTS questions (farm_id TEXT PRIMARY KEY NOT NULL, hectors TEXT,region_id TEXT,district TEXT,area_name TEXT,address TEXT,physical_address TEXT,epa TEXT, crop_id TEXT, variety_id TEXT,grower_id TEXT, FOREIGN KEY(crop_id)  REFERENCES crop(crop_id),FOREIGN KEY(variety_id)  REFERENCES variety(variety_id),FOREIGN KEY(grower_id) REFERENCES growers(grower_id))',
    [],
    () => {
...
db.transaction((tx) => {
  tx.executeSql(
    'CREATE TABLE IF NOT EXISTS hospital (inspection_id TEXT ,latitude TEXT, longitude TEXT,altitude TEXT,accuracy TEXT,speed TEXT,FOREIGN KEY(inspection_id) REFERENCES inspection(inspection_id))',
    [],
    () => {
